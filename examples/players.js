...
function run(){
    try{
        ...
        players.searchPlayer('greep').then(p=>{
            ...
            players.player(p[0].player.id).then(data=>{
                console.log('data', data)
            }).catch(err=>{
                console.error(err)
            })

            players.playerTrophies(p[0].player.id).then(data=>{
                console.log('last trophies gains', data[0])
            }).catch(err=>{
                console.error(err)
            })

            players.playerMatches(p[0].player.id, "3v3").then(data=>{
                console.log('last match in 3v3 mode', data[0])
            }).catch(err=>{
                console.error(err)
            })

            players.playerMatches(p[0].player.id, "Royal").then(data=>{
